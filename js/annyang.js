...
(function (undefined) {
  ...
  var root_console_log = function(msg){
  ...
  var registerCommand = function(command, cb, phrase) {
    ...
    if (debugState) {
      root_console_log('Command successfully loaded: '+phrase, debugStyle);
    }
  };
  ...
  root.annyang = {
    ...
    init: function(commands, resetCommands) {
      ...
      recognition.onresult  = function(event) {
        if(pauseListening) {
          if (debugState) {
            root_console_log('Speech heard, but annyang is paused');
          }
          return false;
        }
        ...
        for (var i = 0; i<results.length; i++) {
          ...
          if (debugState) {
            root_console_log('Speech recognized: '+commandText, debugStyle);
          }
          ...
          for (var j = 0, l = commandsList.length; j < l; j++) {
            ...
            if (result) {
              ...
              if (debugState) {
                root_console_log('command matched: '+commandsList[j].originalPhrase, debugStyle);
                if (parameters.length) {
                  root_console_log('with parameters', parameters);
                }
              }
      ...
      for (var phrase in commands) {
        if (commands.hasOwnProperty(phrase)) {
          ...
          if (typeof cb === 'function') {
            ...
            if (debugState) {
              root_console_log('Can not register command: '+phrase, debugStyle);
            }
            continue;
          }
        }
